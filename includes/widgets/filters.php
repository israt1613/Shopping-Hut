...
?>
<h3 class="text-left"><b>Search By:</b></h3>
<h4 class="text-left"><b>Price:</b></h4>
<form action="search.php" method="post">
	...
	<h4 class="text-left"><b>Price Range:</b></h4>
    <input type="text" name="min_price" class="price-range" placeholder="Min $" id="textBar" value="<?=$min_price;?>"> To <input type="text" name="max_price" class="price-range" placeholder="Max $" id="textBar" value="<?=$max_price;?>"><br><br>
	<h4 class="text-left"><b>Brand:</b></h4>
	<input type="radio" name="brand" value=""<?=(($db =='')?' checked':'');?>>All<br>
	<?php while ($brand = mysqli_fetch_assoc($brandQ)): ?>
		<input type="radio" name="brand" value="<?=$brand['id'];?>"<?=(($b == $brand['id'])?' checked':'');?>><?=$brand['brand'];?><br>
	<?php endwhile; ?>
	<br><input type="submit" value="Search" class="btn btn-primary" style="height:35px; width:80px;">
</form>
